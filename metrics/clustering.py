...
def spectral_clustering_metrics(A, nclass, label, verbose=True, n_init=10, normalize_embed=True, solver_type='lm', extra_dim=0, tol=0):
    ...
    if solver_type=='shift_invert':
        ...
        n = lap.shape[0]
        identity = scipy.sparse.identity(n, format='csr')
        lap_modified = 2 * identity - lap
        vals, embedding = scipy.sparse.linalg.eigsh(lap_modified, k=nclass+extra_dim, 
                                                  sigma=None, which='LM', tol=tol)
    else:
        raise ValueError('invalid solver')
    
    if normalize_embed:
        row_norms = np.sqrt(np.sum(embedding * embedding, axis=1))
    ...
    batch_size = min(1000, embedding.shape[0])
    ...
    return acc_lst, nmi_lst, pred_lst