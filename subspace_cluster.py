...
import tqdm
from model.DSCNet import SSAG_DSC
...
parser = argparse.ArgumentParser(description='SSAG-DSC Training')
...
model = SSAG_DSC(hidden_dim=args.hidden_dim, z_dim=args.z_dim,channels=args.channels,kernels=args.kernels).to(device)
sink_layer = SinkhornDistance(args.pieta, max_iter=1)

datasets_list = ['coil100','orl','orl_zeroed',
                'coil100_zeroed','coil100_zeroed_subspace_gnn_imputed','coil100_zeroed_20_subspace_gnn_imputed','coil100_zeroed_30_subspace_gnn_imputed','coil100_zeroed_40_subspace_gnn_imputed',
                'coil100_zeroed_50_subspace_gnn_imputed','orl_zeroed_20_subspace_gnn_imputed','orl_zeroed_30_subspace_gnn_imputed','orl_zeroed_40_subspace_gnn_imputed',
                'orl_zeroed_50_subspace_gnn_imputed','orl_zeroed_subspace_gnn_imputed']
...
if args.data.lower() == 'orl':
...
elif args.data.lower() == 'orl_zeroed':
    # Loading features and labels
    data = sio.loadmat(args.data_dir)
    x, y = data['fea'].reshape((-1, 1, 32, 32)), data['gnd']
    y = np.squeeze(y - 1)  # y in [0, 1, ..., K-1]
...
elif args.data.lower() == 'orl_zeroed_subspace_gnn_imputed':
    # Loading features and labels
    data = sio.loadmat(args.data_dir)
    x, y = data['fea'].reshape((-1, 1, 32, 32)), data['gnd']
    y = np.squeeze(y - 1)  # y in [0, 1, ..., K-1]
    ...
    num_sample = x.shape[0]
    temp = model.pre_feature.load_state_dict(torch.load('DSCNet_AE_pretrain/orl.pkl'),strict=False)
elif args.data.lower() == 'orl_zeroed_20_subspace_gnn_imputed':
    # Loading features and labels
    data = sio.loadmat(args.data_dir)
    x, y = data['fea'].reshape((-1, 1, 32, 32)), data['gnd']
    y = np.squeeze(y - 1)  # y in [0, 1, ..., K-1]
    ...
    num_sample = x.shape[0]
    temp = model.pre_feature.load_state_dict(torch.load('DSCNet_AE_pretrain/orl.pkl'),strict=False)
elif args.data.lower() == 'orl_zeroed_30_subspace_gnn_imputed':
    # Loading features and labels
    data = sio.loadmat(args.data_dir)
    x, y = data['fea'].reshape((-1, 1, 32, 32)), data['gnd']
    y = np.squeeze(y - 1)  # y in [0, 1, ..., K-1]
    ...
    num_sample = x.shape[0]
    temp = model.pre_feature.load_state_dict(torch.load('DSCNet_AE_pretrain/orl.pkl'),strict=False)
elif args.data.lower() == 'orl_zeroed_40_subspace_gnn_imputed':
    # Loading features and labels
    data = sio.loadmat(args.data_dir)
    x, y = data['fea'].reshape((-1, 1, 32, 32)), data['gnd']
    y = np.squeeze(y - 1)  # y in [0, 1, ..., K-1]
    ...
    num_sample = x.shape[0]
    temp = model.pre_feature.load_state_dict(torch.load('DSCNet_AE_pretrain/orl.pkl'),strict=False)
elif args.data.lower() == 'orl_zeroed_50_subspace_gnn_imputed':
    # Loading features and labels
    data = sio.loadmat(args.data_dir)
    x, y = data['fea'].reshape((-1, 1, 32, 32)), data['gnd']
    y = np.squeeze(y - 1)  # y in [0, 1, ..., K-1]
...
elif args.data.lower() == 'coil100_zeroed':
    data = sio.loadmat(args.data_dir)
    x, y = data['fea'].reshape((-1, 1, 32, 32)), data['gnd']
    y = np.squeeze(y - 1)  # y in [0, 1, ..., K-1]
    num_sample = x.shape[0]
...
elif args.data.lower() == 'coil100_zeroed_subspace_gnn_imputed':
    data = sio.loadmat(args.data_dir)
    x, y = data['fea'].reshape((-1, 1, 32, 32)), data['gnd']
    y = np.squeeze(y - 1)  # y in [0, 1, ..., K-1]
    num_sample = x.shape[0]
    temp = model.pre_feature.load_state_dict(torch.load('DSCNet_AE_pretrain/coil100.pkl'),strict=False)
elif args.data.lower() == 'coil100_zeroed_20_subspace_gnn_imputed':
    data = sio.loadmat(args.data_dir)
    x, y = data['fea'].reshape((-1, 1, 32, 32)), data['gnd']
    y = np.squeeze(y - 1)  # y in [0, 1, ..., K-1]
    num_sample = x.shape[0]
    temp = model.pre_feature.load_state_dict(torch.load('DSCNet_AE_pretrain/coil100.pkl'),strict=False)
elif args.data.lower() == 'coil100_zeroed_30_subspace_gnn_imputed':
    data = sio.loadmat(args.data_dir)
    x, y = data['fea'].reshape((-1, 1, 32, 32)), data['gnd']
    y = np.squeeze(y - 1)  # y in [0, 1, ..., K-1]
    num_sample = x.shape[0]
    temp = model.pre_feature.load_state_dict(torch.load('DSCNet_AE_pretrain/coil100.pkl'),strict=False)
elif args.data.lower() == 'coil100_zeroed_40_subspace_gnn_imputed':
    data = sio.loadmat(args.data_dir)
    x, y = data['fea'].reshape((-1, 1, 32, 32)), data['gnd']
    y = np.squeeze(y - 1)  # y in [0, 1, ..., K-1]
    num_sample = x.shape[0]
    temp = model.pre_feature.load_state_dict(torch.load('DSCNet_AE_pretrain/coil100.pkl'),strict=False)
elif args.data.lower() == 'coil100_zeroed_50_subspace_gnn_imputed':
    data = sio.loadmat(args.data_dir)
    x, y = data['fea'].reshape((-1, 1, 32, 32)), data['gnd']
    y = np.squeeze(y - 1)  # y in [0, 1, ..., K-1]
    num_sample = x.shape[0]
...
with tqdm.tqdm(total=args.epo) as progress_bar:
