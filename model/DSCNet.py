...
class DSCNet(nn.Module):
    ...
    def forward(self, x):  # shape=[n, c, w, h]
        ...
        # shape = q.shape
        q = q.view(x.shape[0], -1)  # shape=[n, d]
        # q_recon = self.self_expression(q)  # shape=[n, d]
        # q_recon_reshape = q_recon.view(shape)

        # x_recon = self.ae.decoder(q_recon_reshape)  # shape=[n, c, w, h]
        return q

    def loss_fn(self, x, x_recon, q, q_recon, weight_coef, weight_selfExp):
        loss_ae = 0.5 * F.mse_loss(x_recon, x, reduction='sum')
        loss_coef = torch.sum(torch.pow(self.self_expression.Coefficient, 2))
        loss_selfExp = 0.5 * F.mse_loss(q_recon, q, reduction='sum')
